...
/* ── Executive Summary — Domain tiles ── */
.dom-tile {
  background: var(--white); border:1px solid var(--border);
  border-radius:12px; padding:16px 16px 14px;
  box-shadow:var(--shadow-sm); position:relative; overflow:hidden;
  transition:box-shadow 0.18s, transform 0.18s; min-height:138px;
}
.dom-tile:hover { box-shadow:var(--shadow-md); transform:translateY(-1px); }
.dom-name { font-family:'DM Serif Display',serif; font-size:0.97rem; color:var(--slate-900); margin:5px 0 4px; }
.dom-val  { font-family:'JetBrains Mono',monospace; font-size:1.08rem; font-weight:600; line-height:1.15; margin-bottom:3px; }
.dom-ref  { font-family:'JetBrains Mono',monospace; font-size:0.57rem; color:var(--slate-400); letter-spacing:0.12em; text-transform:uppercase; margin-bottom:8px; }

/* ── Alert banners ── */
.abreach { background:var(--crimson-bg); border:1px solid rgba(153,27,27,0.25);
  border-left:4px solid var(--crimson); border-radius:8px;
  padding:10px 16px; margin-bottom:1.2rem;
  font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--crimson); line-height:1.65; }
.awarn { background:var(--amber-bg); border:1px solid rgba(146,64,14,0.25);
  border-left:4px solid var(--amber); border-radius:8px;
  padding:10px 16px; margin-bottom:1.2rem;
  font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--amber); line-height:1.65; }
.aclear { background:var(--green-bg); border:1px solid rgba(22,101,52,0.20);
  border-left:4px solid var(--green); border-radius:8px;
  padding:10px 16px; margin-bottom:1.2rem;
  font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--green); line-height:1.65; }

/* ── Limit mini tile ── */
.lmini { background:var(--white); border:1px solid var(--border); border-radius:10px;
  padding:14px 14px 12px; box-shadow:var(--shadow-sm); }
.lmini-lbl { font-family:'JetBrains Mono',monospace; font-size:0.57rem; color:var(--slate-400);
  letter-spacing:0.14em; text-transform:uppercase; margin-bottom:5px; }
.lmini-val { font-family:'JetBrains Mono',monospace; font-size:1.05rem; font-weight:600; color:var(--slate-900); margin-bottom:2px; }
.lmini-cap { font-family:'Inter',sans-serif; font-size:0.69rem; color:var(--slate-400); margin-bottom:7px; }
...
with st.sidebar:
    ...
    st.markdown('<div class="submark">Basel III/IV · Risk Intelligence Platform</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'margin-top:-8px;margin-bottom:16px">{run_date.strftime("%A, %d %b %Y")}</div>',
        unsafe_allow_html=True)

    # ── Navigation group headers ──
    def _nav_sep(label):
        st.markdown(
            f'<div style="font-family:JetBrains Mono;font-size:0.56rem;color:#c9c2b6;'
            f'letter-spacing:0.18em;text-transform:uppercase;padding:8px 4px 4px;'
            f'border-top:1px solid #f0ede7;margin-top:4px">{label}</div>',
            unsafe_allow_html=True)
    ...
    page = st.radio("", [
        ...
        "Banking Book",
        "Market Risk (FRTB)",
        "IMM Exposure Profiles",
        "CVA Risk", "CCP Exposure",
        "Operational Risk",
        "Risk Limits", "Backtesting",
        "Reports", "Market Calibration",
    ...
    for code, desc in [
        ...
        ("CRE54","CCP Exposure"),("RBC20","Capital Agg."),("OPE25","Op Risk SMA"),
    ]:
        st.markdown(
            f'<div class="reg-row">'
...
# ── Sidebar live status panel (requires loaded data) ─────────────────────────
with st.sidebar:
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        '<div style="font-family:JetBrains Mono;font-size:0.56rem;color:#c9c2b6;'
        'letter-spacing:0.18em;text-transform:uppercase;padding:4px 4px 6px;'
        'border-top:1px solid #f0ede7">Live Status</div>',
        unsafe_allow_html=True)
    _zone = bt.get("traffic_light", "GREEN")
    _status_rows = [
        ("CET1",     fmt_pct(cap["cet1_ratio"]),
         "t-ok" if cap["cet1_ratio"] > 0.07 else ("t-warn" if cap["cet1_ratio"] > 0.045 else "t-bad")),
        ("Tier 1",   fmt_pct(cap["tier1_ratio"]),
         "t-ok" if cap["tier1_ratio"] > 0.08 else "t-bad"),
        ("Total Cap",fmt_pct(cap["total_cap_ratio"]),
         "t-ok" if cap["total_cap_ratio"] > 0.10 else "t-bad"),
        ("Total RWA",fmt_bn(cap["rwa_total"]),  "t-stone"),
        ("BT Zone",  _zone,
         {"GREEN":"t-ok","AMBER":"t-warn","RED":"t-bad"}.get(_zone,"t-ok")),
    ]
    _TAG_STYLE = {
        "t-ok":   ("rgba(22,101,52,0.065)",  "#166534"),
        "t-warn": ("rgba(146,64,14,0.065)", "#92400e"),
        "t-bad":  ("rgba(153,27,27,0.055)", "#991b1b"),
        "t-stone":("#f7f6f3",               "#64748b"),
        "t-blue": ("rgba(30,64,175,0.055)", "#1e40af"),
    }
    for _lbl, _val, _cls in _status_rows:
        _bg, _clr = _TAG_STYLE.get(_cls, ("#f7f6f3", "#64748b"))
        st.markdown(
            f'<div class="sb-stat" style="background:{_bg}">'
            f'<span style="font-family:JetBrains Mono;font-size:0.67rem;color:#94a3b8">{_lbl}</span>'
            f'<span style="font-family:JetBrains Mono;font-size:0.69rem;font-weight:600;color:{_clr}">{_val}</span>'
            f'</div>', unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# 0 · EXECUTIVE SUMMARY  (CRO Morning Pack)
# ══════════════════════════════════════════════════════════════════════════════
if page == "Executive Summary":
    st.markdown('<div class="page-title">Executive Risk Summary</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="page-sub">G-SIB Daily Risk Pack · Basel III/IV · {run_date.strftime("%A, %d %b %Y")}'
        f' · All figures in USD unless stated</div>', unsafe_allow_html=True)

    # ── Alert banner ──────────────────────────────────────────────────────────
    _zone = bt.get("traffic_light", "GREEN")
    _breaches, _warnings = [], []
    if cap["cet1_ratio"] < 0.045:
        _breaches.append(f"CET1 {cap['cet1_ratio']*100:.2f}% BELOW 4.5% regulatory minimum")
    elif cap["cet1_ratio"] < 0.07:
        _warnings.append(f"CET1 {cap['cet1_ratio']*100:.2f}% — approaching conservation buffer")
    if cap["total_cap_ratio"] < 0.08:
        _breaches.append(f"Total Capital {cap['total_cap_ratio']*100:.2f}% BELOW 8.0% minimum")
    if _zone == "RED":
        _breaches.append(f"Backtesting RED zone — {bt.get('exceptions',0)} exceptions (model revocation risk)")
    elif _zone == "AMBER":
        _warnings.append(f"Backtesting AMBER — {bt.get('exceptions',0)} exceptions (supervisory add-on applied)")
    if cap.get("floor_triggered"):
        _warnings.append("Output Floor binding (RBC20.11) — SA-based floor exceeds internal model RWA")
    if _breaches:
        st.markdown(
            f'<div class="abreach">⚠  {len(_breaches)} REGULATORY LIMIT BREACH{"ES" if len(_breaches)>1 else ""} DETECTED'
            f'<br>{"<br>".join(f"  ·  {b}" for b in _breaches)}</div>', unsafe_allow_html=True)
    elif _warnings:
        st.markdown(
            f'<div class="awarn">⚑  {len(_warnings)} EARLY WARNING INDICATOR{"S" if len(_warnings)>1 else ""}'
            f'<br>{"<br>".join(f"  ·  {w}" for w in _warnings)}</div>', unsafe_allow_html=True)
    else:
        st.markdown(
            f'<div class="aclear">✓  All capital ratios, backtesting and limits within tolerance'
            f'  ·  Output floor not triggered  ·  {run_date.strftime("%d %b %Y")}</div>',
            unsafe_allow_html=True)

    # ── Capital adequacy KPIs ─────────────────────────────────────────────────
    sec("CAPITAL ADEQUACY — RBC20")
    c1,c2,c3,c4,c5 = st.columns(5)
    kpi(c1,"CET1 Ratio",    fmt_pct(cap["cet1_ratio"]),       "Common Equity Tier 1",  "≥ 4.5% min",
        "t-ok" if cap["cet1_ratio"]>0.07 else ("t-warn" if cap["cet1_ratio"]>0.045 else "t-bad"))
    kpi(c2,"Tier 1 Ratio",  fmt_pct(cap["tier1_ratio"]),      "Tier 1 Capital",         "≥ 6.0% min",
        "t-ok" if cap["tier1_ratio"]>0.08 else "t-bad")
    kpi(c3,"Total Capital", fmt_pct(cap["total_cap_ratio"]),  "Total Capital Ratio",    "≥ 8.0% min",
        "t-ok" if cap["total_cap_ratio"]>0.10 else "t-bad")
    kpi(c4,"Total RWA",     fmt_bn(cap["rwa_total"]),         "Risk-Weighted Assets",   "5-Part RBC20","t-blue")
    _ztag = {"GREEN":"t-ok","AMBER":"t-warn","RED":"t-bad"}.get(_zone,"t-ok")
    kpi(c5,"Backtesting",
        {"GREEN":"● GREEN","AMBER":"● AMBER","RED":"● RED"}.get(_zone,"● GREEN"),
        "MAR99 Traffic Light", f"{bt.get('exceptions',0)} exceptions / {bt.get('window_days',250)}d", _ztag)

    # ── Six risk domain tiles ─────────────────────────────────────────────────
    sec("RISK DOMAIN STATUS")
    _total_bbk_ead = sum(p["total_ead"] for p in bbk)
    _total_der_ead = sum(p["saccr"]["ead"] for p in drv)
    _domains = [
        ("#1e40af", "📘", "Credit Risk",     fmt_bn(cap["rwa_credit"]),       "A-IRB · CRE30–36",    "t-blue"),
        ("#3730a3", "📊", "Market Risk",      fmt_bn(cap["rwa_market"]),       "FRTB · MAR21–33",     "t-blue"),
        ("#0f766e", "🔄", "CCR",              fmt_bn(cap["rwa_ccr"]),          "SA-CCR/IMM · CRE52–53","t-teal"),
        ("#92400e", "📉", "CVA Risk",         fmt_bn(cap.get("rwa_cva",0)),    "BA/SA-CVA · MAR50",   "t-warn"),
        ("#166534", "🏦", "CCP Exposure",     fmt_bn(cap.get("rwa_ccp",0)),    "QCCP · CRE54",        "t-ok"),
        ({"GREEN":"#166534","AMBER":"#92400e","RED":"#991b1b"}.get(_zone,"#166534"),
         "📋", "Backtesting",
         f"● {_zone}",
         f"MAR99 · {bt.get('exceptions',0)} exceptions",
         {"GREEN":"t-ok","AMBER":"t-warn","RED":"t-bad"}.get(_zone,"t-ok")),
    ]
    _dcols = st.columns(6)
    for _col, (_clr, _ico, _name, _val, _ref, _tag) in zip(_dcols, _domains):
        with _col:
            st.markdown(
                f'<div class="dom-tile">'
                f'<div style="position:absolute;top:0;left:0;right:0;height:3px;'
                f'background:{_clr};border-radius:12px 12px 0 0"></div>'
                f'<div style="font-size:1.3rem;margin-bottom:4px">{_ico}</div>'
                f'<div class="dom-name">{_name}</div>'
                f'<div class="dom-val" style="color:{_clr}">{_val}</div>'
                f'<div class="dom-ref">{_ref}</div>'
                f'<span class="kpi-tag {_tag}">RWA</span>'
                f'</div>', unsafe_allow_html=True)

    # ── RWA composition + Capital gauges ─────────────────────────────────────
    st.markdown("")
    sec("RWA COMPOSITION — FIVE COMPONENTS (RBC20.9)")
    _comps = ["Credit (A-IRB)","CCR (SA-CCR/IMM)","Market (FRTB)","CVA (MAR50)","CCP (CRE54)"]
    _vals  = [cap["rwa_credit"],cap["rwa_ccr"],cap["rwa_market"],
              cap.get("rwa_cva",0),cap.get("rwa_ccp",0)]
    _pie_col, _g1, _g2, _g3 = st.columns([3,2,2,2])
    with _pie_col:
        _fig_pie = go.Figure(go.Pie(
            labels=_comps, values=_vals,
            marker=dict(colors=CHART_COLS, line=dict(color="#ffffff", width=2)),
            hole=0.55, textinfo="percent",
            textfont=dict(family="JetBrains Mono", size=10),
            hovertemplate="<b>%{label}</b><br>%{customdata}<extra></extra>",
            customdata=[fmt_bn(v) for v in _vals],
        ))
        _fig_pie.add_annotation(
            text=fmt_bn(sum(_vals)), x=0.5, y=0.5, showarrow=False,
            font=dict(family="JetBrains Mono", size=13, color="#0f172a"))
        _fig_pie.update_layout(**PLOT(300, 0), showlegend=True, legend=LEG("v", 0.5, 1.02))
        st.plotly_chart(_fig_pie, use_container_width=True)
    with _g1: st.plotly_chart(gauge(cap["cet1_ratio"],0.045,"CET1 Ratio"), use_container_width=True)
    with _g2: st.plotly_chart(gauge(cap["tier1_ratio"],0.060,"Tier 1 Ratio"), use_container_width=True)
    with _g3: st.plotly_chart(gauge(cap["total_cap_ratio"],0.080,"Total Capital"), use_container_width=True)

    # ── Limit utilisation mini tiles ─────────────────────────────────────────
    sec("RISK LIMIT UTILISATION — REAL-TIME MONITORING")
    _limits = [
        ("CET1 Ratio",    4.5,  "%", cap["cet1_ratio"]*100,                            0.95),
        ("Total Capital", 8.0,  "%", cap["total_cap_ratio"]*100,                       0.92),
        ("FRTB ES (10d)", 100,  "M", sum(p["frtb"]["es_10d"] for p in drv)/1e6,        0.80),
        ("CCR EAD",       500,  "M", sum(p["saccr"]["ead"] for p in drv)/1e6,          0.80),
        ("Credit RWA",    1500, "M", cap["rwa_credit"]/1e6,                            0.80),
        ("CVA RWA",       50,   "M", cap.get("rwa_cva",0)/1e6,                         0.80),
    ]
    _lm_cols = st.columns(6)
    for _lc, (_ln, _llv, _lu, _lc_val, _lw) in zip(_lm_cols, _limits):
        _util = _lc_val / _llv
        _pct  = min(_util * 100, 100)
        _bar  = C["red"] if _util>=1 else (C["amber"] if _util>=_lw else C["green"])
        _stat = "BREACH" if _util>=1 else ("WARN" if _util>=_lw else "OK")
        _sbg, _sclr = (
            ("rgba(153,27,27,0.055)","#991b1b") if _util>=1 else
            (("rgba(146,64,14,0.065)","#92400e") if _util>=_lw else
             ("rgba(22,101,52,0.065)","#166534"))
        )
        with _lc:
            st.markdown(
                f'<div class="lmini">'
                f'<div class="lmini-lbl">{_ln}</div>'
                f'<div class="lmini-val">{_lc_val:.1f}{_lu}</div>'
                f'<div class="lmini-cap">of {_llv:.0f}{_lu} limit</div>'
                f'<div class="lmini-bar"><div style="width:{_pct:.1f}%;height:4px;'
                f'background:{_bar};border-radius:3px"></div></div>'
                f'<span style="font-family:JetBrains Mono;font-size:0.62rem;font-weight:500;'
                f'padding:2px 7px;border-radius:4px;background:{_sbg};color:{_sclr};'
                f'border:1px solid {_sclr}22">{_util*100:.0f}%  {_stat}</span>'
                f'</div>', unsafe_allow_html=True)

    # ── Output floor + context ────────────────────────────────────────────────
    st.markdown("")
    _floor_rwa = cap.get("rwa_floor", cap.get("rwa_sa_based", cap["rwa_total"])*0.725)
    _pre_floor = cap.get("rwa_total_pre_floor", cap["rwa_total"])
    if cap.get("floor_triggered"):
        st.markdown(
            f'<div class="floor-warn">⚠  Output Floor Triggered (RBC20.11) — '
            f'Pre-floor {fmt_bn(_pre_floor)} &lt; Floor {fmt_bn(_floor_rwa)}</div>',
            unsafe_allow_html=True)
    else:
        st.markdown(
            f'<div class="floor-ok">✓  Output Floor not triggered — '
            f'Pre-floor {fmt_bn(_pre_floor)} ≥ 72.5% SA Floor {fmt_bn(_floor_rwa)}'
            f'<span style="color:{C["teal"]};margin-left:14px;font-family:JetBrains Mono;font-size:0.68rem">'
            f'CVA excluded from floor base · CAP10 FAQ1</span></div>',
            unsafe_allow_html=True)

    # ── Capital stack summary ─────────────────────────────────────────────────
    sec("CAPITAL STACK SUMMARY")
    _cs1,_cs2,_cs3,_cs4 = st.columns(4)
    for _col, _acc, _val, _ratio, _minr, _colour in [
        (_cs1,"CET1 Capital",   cap["cet1_capital"],   cap["cet1_ratio"],      0.045, C["blue"]),
        (_cs2,"Tier 1 Capital", cap["tier1_capital"],  cap["tier1_ratio"],     0.060, C["indigo"]),
        (_cs3,"Total Capital",  cap["total_capital"],  cap["total_cap_ratio"], 0.080, C["teal"]),
        (_cs4,"Min Req (8%)",   cap["rwa_total"]*0.08, None, None,                   C["red"]),
    ]:
        _rt  = f" · {_ratio*100:.1f}%" if _ratio else ""
        _mn  = f"Min {_minr*100:.1f}%" if _minr else "Regulatory floor"
        _cls = "t-ok" if (_ratio and _ratio>=_minr) else ("t-bad" if _ratio else "t-warn")
        with _col:
            st.markdown(
                f'<div class="kpi" style="border-top:2px solid {_colour}">'
                f'<div class="kpi-sup">{_acc}</div>'
                f'<div class="kpi-val" style="color:{_colour}">{fmt_bn(_val)}</div>'
                f'<div class="kpi-lbl">{_acc}{_rt}</div>'
                f'<span class="kpi-tag {_cls}">{_mn}</span></div>',
                unsafe_allow_html=True)

    # ── Regulatory framework strip ────────────────────────────────────────────
    sec("ACTIVE REGULATORY FRAMEWORKS")
    st.markdown("""
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;font-size:0.78rem">
<div style="background:#fff;border:1px solid #dedad1;border-radius:10px;padding:12px 14px">
  <div style="font-family:'JetBrains Mono',monospace;font-size:0.62rem;color:#991b1b;font-weight:500;letter-spacing:0.1em;margin-bottom:4px">CREDIT RISK</div>
  <div style="color:#334155;font-weight:500;margin-bottom:2px">A-IRB · CRE30–36</div>
  <div style="color:#94a3b8;font-size:0.73rem">Vasicek formula · Frye-Jacobs LGD · Double-default CDS</div>
</div>
<div style="background:#fff;border:1px solid #dedad1;border-radius:10px;padding:12px 14px">
  <div style="font-family:'JetBrains Mono',monospace;font-size:0.62rem;color:#0f766e;font-weight:500;letter-spacing:0.1em;margin-bottom:4px">MARKET RISK</div>
  <div style="color:#334155;font-weight:500;margin-bottom:2px">FRTB SBM + IMA · MAR21–33</div>
  <div style="color:#94a3b8;font-size:0.73rem">ES 97.5% · Delta/Vega/Curvature · MAR99 backtesting</div>
</div>
<div style="background:#fff;border:1px solid #dedad1;border-radius:10px;padding:12px 14px">
  <div style="font-family:'JetBrains Mono',monospace;font-size:0.62rem;color:#3730a3;font-weight:500;letter-spacing:0.1em;margin-bottom:4px">COUNTERPARTY</div>
  <div style="color:#334155;font-weight:500;margin-bottom:2px">SA-CCR · IMM MC · CVA · CCP</div>
  <div style="color:#94a3b8;font-size:0.73rem">CRE52/53 · MAR50 BA/SA-CVA · CRE54 QCCP</div>
</div>
<div style="background:#fff;border:1px solid #dedad1;border-radius:10px;padding:12px 14px">
  <div style="font-family:'JetBrains Mono',monospace;font-size:0.62rem;color:#92400e;font-weight:500;letter-spacing:0.1em;margin-bottom:4px">OPERATIONAL</div>
  <div style="color:#334155;font-weight:500;margin-bottom:2px">SMA · OPE25 · BIC × ILM</div>
  <div style="color:#94a3b8;font-size:0.73rem">Five-tier BI structure · 10-year loss history · ILM cap 1.0</div>
</div>
